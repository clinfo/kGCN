        ...
        **kwargs
        ...
        with tf.variable_scope("graph_nn") as scope_part:
            ...
            for i in range(int(info.param["num_gcn_layer"])):
                ...
                ###
                layer = kgcn.layers.GraphConv(out_dim, self.adj_channel_num)(
                    layer, adj=in_adjs
                )
        ...
        with tf.variable_scope("seq_nn") as scope_part:
            ...
            self.embedding_layer = tf.keras.layers.Embedding(
                self.sequence_symbol_num, self.embedding_dim
            )(sequences)
            ...
            for i in range(int(info.param["num_seq_layer"])):
                ...
                layer = tf.keras.layers.Conv1D(
                    out_dim, stride, padding="same", activation="relu"
                )(layer)
                layer = tf.keras.layers.MaxPooling1D(stride)(layer)
                in_dim = out_dim
            stride = 4
            layer = tf.keras.layers.Conv1D(
                1, stride, padding="same", activation="tanh"
            )(layer)
