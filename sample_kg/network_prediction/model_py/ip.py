...
class KGNetwork(DefaultModel):
    ...
    def build_model(self, placeholders, info, config, batch_size=4,feed_embedded_layer=False,**kwargs):
        ...
        lp_prediction=tf.matmul(prediction,prediction,transpose_b=True)
        ...
        s1=tf.reduce_sum(pred0*pred1)
        s2=tf.reduce_sum(pred2*pred3)
        gamma=0.1
        score = s2 - s1+gamma
        ...
        #left_pred=(prediction[0,:,:],pred1,label_list[0,:,1])
        ####
