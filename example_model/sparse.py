...
def build(config):
    ...
    estimator_config = tf.estimator.RunConfig(session_config=session_config,
                                              save_checkpoints_steps=config["steps_per_epoch"])
    max_degree = 0 if config["normalize_adj_flag"] or not(config["split_adj_flag"]) else 5
...
def sparse_model(features, labels, mode, params):
    ...
    for out_dim in params["out_dims"]:
        ...
        if params["max_pool"]:
            net = layers.GraphMaxPooling(params["max_degree"] + 1)(net, diagonalized_adj)
    ...
    with tf.device("/cpu:0"):
        ...
        def split_sum(a, x):
            mol_gathered = tf.reduce_sum(net[x[0]: x[0] + x[1]], axis=0, keepdims=False)
            return mol_gathered
    ...
    metrics = {"accuracy": tf.metrics.accuracy(labels, tf.math.argmax(probabilities, axis=1))}
    if params["num_classes"] > 100:
        metrics["top30"] = tf.metrics.mean(tf.nn.in_top_k(predictions=probabilities,
                                                          targets=tf.cast(labels, tf.int32),
                                                          k=30))
        metrics["top50"] = tf.metrics.mean(tf.nn.in_top_k(predictions=probabilities,
                                                          targets=tf.cast(labels, tf.int32),
                                                          k=50))
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        with tf.control_dependencies(update_ops):
            train_op = optimizer.minimize(loss=loss_to_minimize, global_step=tf.train.get_global_step())
        return tf.estimator.EstimatorSpec(mode=mode, loss=loss_to_minimize, train_op=train_op)

    if mode == tf.estimator.ModeKeys.EVAL:
        return tf.estimator.EstimatorSpec(mode=mode, loss=loss_to_minimize, eval_metric_ops=metrics)
