...
if [ $method = "ip" ]; then
    mkdir -p result_ip
    echo "\n-train mode"
    python ../../gcn.py train --config ./config/config_ip.json --cpu
    echo "\n-infer mode"
    python ../../gcn.py infer --config ./config/config_ip.json --cpu

elif [ $method = "distmult" ]; then
    mkdir -p result_distmult
    echo "\n-train mode"
    python ../../gcn.py train --config ./config/config_distmult.json --cpu
    echo "\n-infer mode"
    python ../../gcn.py infer --config ./config/config_distmult.json --cpu

elif [ $method = "gcn" ]; then
    mkdir -p result_gcn
    echo "\n-train mode"
    python ../../gcn.py train --config ./config/config_gcn.json --cpu
    echo "\n-infer mode"
    python ../../gcn.py infer --config ./config/config_gcn.json --cpu
else
    echo "[ERROR]: Select methods."
fi
